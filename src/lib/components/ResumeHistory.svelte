								...
								<div class="min-w-0">
									<div class="flex items-center gap-2">
										...
										{#if entry.application_id}
											<span
												class="inline-flex shrink-0 items-center gap-1 rounded-full bg-primary-500/15 px-1.5 py-0.5 text-[10px] font-medium text-primary-500"
												title="Generated by the apply pipeline"
											>
												<RocketIcon class="size-2.5" />
												Pipeline
											</span>
										{/if}
									</div>
									<p class="mt-0.5 text-xs opacity-50">
										...
										{#if entry.application_id}
											<span class="mx-1">·</span>
											<a
												href={resolve(`/applications/${entry.application_id}`)}
												class="inline-flex items-center gap-0.5 text-primary-500 hover:underline"
